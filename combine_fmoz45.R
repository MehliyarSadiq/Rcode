...
combine<-function(case.name) {
  ...
  # combine psn
  name.tmp = paste("fmoz_clm45_FSUN_",case.name,".nc", sep="")
  ...
  name.tmp = paste("fmoz_clm45_PSNSUN_",case.name,".nc", sep="")
  file.tmp = open.ncdf(name.tmp,write=FALSE)
  psnsun = get.var.ncdf(file.tmp,"PSNSUN")
  unit = att.get.ncdf(file.tmp,"PSNSUN","units")$value
  close.ncdf(file.tmp)
  
  name.tmp = paste("fmoz_clm45_PSNSHA_",case.name,".nc", sep="")
  file.tmp = open.ncdf(name.tmp,write=FALSE)
  ...
  file.name.tmp = paste("fmoz_clm45_PSN_",case.name,".nc", sep="")
  ncnew=create.ncdf(file.name.tmp,var)
  put.var.ncdf(ncnew,var,psn)
  close.ncdf(ncnew)
  
  # combine rs
  name.tmp = paste("fmoz_clm45_RSSUN_",case.name,".nc", sep="")
  file.tmp = open.ncdf(name.tmp,write=FALSE)
  rssun = get.var.ncdf(file.tmp,"RSSUN")
  unit = att.get.ncdf(file.tmp,"RSSUN","units")$value
  close.ncdf(file.tmp)
  
  name.tmp = paste("fmoz_clm45_RSSHA_",case.name,".nc", sep="")
  file.tmp = open.ncdf(name.tmp,write=FALSE)
  ...
  file.name.tmp = paste("fmoz_clm45_RS_",case.name,".nc", sep="")
  ncnew=create.ncdf(file.name.tmp,var)
